...
function injectPremiumControls() {
    ...
    // Create a container for premium controls
    const controlsContainer = document.createElement('div');
    controlsContainer.className = 'hidden lg:flex items-center gap-4 ml-4 pl-4 border-l border-slate-200 dark:border-slate-700';
    
    // HTML for controls
    controlsContainer.innerHTML = `
        <!-- Search -->
        <button id="btn-global-search" class="text-slate-600 hover:text-brandBlue dark:text-slate-300 dark:hover:text-brandGold transition text-lg" title="Search">
            <i class="fa-solid fa-magnifying-glass"></i>
        </button>
        
        <!-- Notifications -->
        <div class="relative group">
            <button id="btn-notifications" class="text-slate-600 hover:text-brandBlue dark:text-slate-300 dark:hover:text-brandGold transition text-lg relative" title="Notifications">
                <i class="fa-solid fa-bell"></i>
                <span class="absolute -top-1 -right-1 w-2.5 h-2.5 bg-red-500 rounded-full animate-pulse"></span>
            </button>
            <div id="notif-dropdown" class="absolute right-0 mt-2 w-72 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50 overflow-hidden transform origin-top-right scale-95 group-hover:scale-100">
                <div class="p-3 border-b border-slate-100 dark:border-slate-700 font-bold text-slate-800 dark:text-white flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
                    <span data-i18n="notif-title">Live Notifications</span>
                    <span class="text-[10px] bg-brandBlue text-white px-2 py-0.5 rounded-full">3 New</span>
                </div>
        ...
        <!-- Language Switcher -->
        <button id="btn-lang" class="text-xs font-bold text-slate-600 hover:text-brandBlue dark:text-slate-300 border border-slate-300 dark:border-slate-600 rounded px-2 py-1 transition">
            HI
        </button>

        <!-- Dark Mode Toggle -->
        <button id="btn-dark-mode" class="text-slate-600 hover:text-brandBlue dark:text-slate-300 dark:hover:text-brandGold transition text-xl w-8 h-8 flex items-center justify-center rounded-full bg-slate-100 dark:bg-slate-800 shadow-inner" title="Toggle Theme">
            <i class="fa-solid fa-moon dark:hidden"></i>
            <i class="fa-solid fa-sun hidden dark:block text-brandGold"></i>
        </button>
    `;

    // Insert before the Login CTA if it exists, otherwise at the end
    ...
    // Add mobile versions in the mobile menu (handling this when mobile menu is opened in main.js is complex, so we just append to body if mobile menu is open, or add a simple bar)
    // For simplicity, we add a floating action button on mobile for these features.
...
function initNotifications() {
    ...
    list.innerHTML = notifications.map(n => `
        <a href="student-corner.html" class="block p-4 border-b border-slate-100 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/30 transition">
            <div class="flex items-start gap-3">
                <div class="w-2 h-2 mt-1.5 rounded-full ${n.type === 'alert' ? 'bg-red-500' : (n.type==='success' ? 'bg-green-500' : 'bg-blue-500')}"></div>
                <div>
                    <h5 class="text-sm font-bold text-slate-800 dark:text-white">${n.title}</h5>
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">${n.desc}</p>
                    <span class="text-[10px] text-slate-400 mt-2 block"><i class="fa-regular fa-clock mr-1"></i>${n.time}</span>
                </div>
            </div>
        </a>
    `).join('') + `
        <a href="student-corner.html" class="block p-3 text-center text-xs font-bold text-brandBlue dark:text-blue-400 hover:underline bg-slate-50 dark:bg-slate-800">
            View All Notices
        </a>
    `;
}
